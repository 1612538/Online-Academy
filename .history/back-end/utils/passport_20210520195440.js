  ...
  new localStrategy(async (username, password, done) => {
    ...
    if (user)
      if (await bcrypt.checkPassword(password, user.password)) {
        ...
        if (user.isBlocked === 1) return done(null, false);
        return done(null, user);
      }
    if (teacher) {
      //kiểm tra giá trị trường có name là teacher
      if (await bcrypt.checkPassword(password, teacher.password)) {
        // kiểm tra giá trị trường có name là password
        if (teacher.isBlocked === 1) return done(null, false);
        return done(null, teacher);
      }
    }

    if (admin) {
      //kiểm tra giá trị trường có name là admin
      if (await bcrypt.checkPassword(password, admin.password)) {
        // kiểm tra giá trị trường có name là password
        return done(null, admin);
      }
    }
